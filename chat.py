...
NIVEIS = {"user": 0, "usuario": 0, "mod": 1, "admin": 2}
...
# ══════════════════════════════════════
def conectar():
    """Funciona com get_conn() retornando Connection OU context manager (pool)."""
...
def enviar_mensagem(dados: MensagemEnvio):
    ...
    with conectar() as conn:
        ...
        if cmd_promover:
            ...
            _inserir_sistema(conn, f"👑 {alvo_nick} agora é {novo_cargo}!", agora)
